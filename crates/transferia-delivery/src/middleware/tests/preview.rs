...
use transferia_core::{DatasetSchema, DiscoveredDataset, SystemColumns};

use super::*;

const BUDGET: MiddlewarePreviewContext = MiddlewarePreviewContext {
...
#[tokio::test]
async fn preview_matches_the_current_identity_after_the_previous_step_renames_it(
) -> anyhow::Result<()> {
    ...
    registry.register_middleware(
        MiddlewareRegistration::new::<BTreeMap<String, String>, _, _>(
            "rename",
            "Rename",
            || serde_json::json!({}),
            |_| Ok(Box::new(Rename)),
    ...
    let preview =
        preview_chain(&steps, sample(vec![1]), 1, BUDGET, CancellationToken::new()).await?;
...
async fn zero_rows_keep_the_native_schema_for_the_next_step() -> anyhow::Result<()> {
    ...
    assert_eq!(preview.after.batch.num_rows(), 0);
    assert_eq!(
        preview.after.batch.schema().field(0).data_type(),
        &DataType::Int64
    );
...
async fn cancellation_drops_the_current_transform_execution() {
    ...
    assert!(format!("{error:#}").contains("cancelled"));
    assert!(
        execution_dropped.is_cancelled(),
        "the running transform must not survive cancellation"
    );
}
...
fn display_keeps_full_integer_and_decimal_precision_as_text() -> anyhow::Result<()> {
    assert_eq!(
        serde_json::to_value(display_rows(&sample(vec![i64::MAX]))?)?,
        serde_json::json!([{"id":"9223372036854775807"}])
    );
    ...
    assert_eq!(
        serde_json::to_value(display_rows(&data)?)?,
        serde_json::json!([{"amount":"12345678901234567890.123456789"}, {"amount":null}])
    );
    Ok(())
}
...
async fn physical_table_columns_are_not_hidden_based_on_metadata_like_names() -> anyhow::Result<()>
{
    ...
    assert_eq!(
        serde_json::to_value(display_rows(&preview.after)?)?,
        serde_json::json!([{"_system_offset":"123"}])
    );
    Ok(())
}
