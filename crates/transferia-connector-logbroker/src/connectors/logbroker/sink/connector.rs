...
pub fn build_sink_connector(config: LogbrokerSinkConfig) -> anyhow::Result<Box<dyn SinkConnector>> {
    config.validate()?;
    match config.driver {
        ...
        LogbrokerDriver::Pqv1 => {
            ...
            Ok(Box::new(
                crate::connectors::logbroker::pqv1::PqV1SinkConnector::from_config(pqv1)?,
            ))
        }
    }
}
