...
async fn build_resolved_endpoints(
    ...
    let sink_config = transferia
        ...
        .resolve(
            sink_kind,
            transferia_providers::extension::EndpointRole::Sink,
...
async fn root_example_config_matches_registered_provider_shapes() -> anyhow::Result<()> {
    let raw = std::fs::read_to_string(workspace_root().join("config.yaml"))?
        .replace("${HOME}", "/tmp")
...
async fn postgres_pipeline_examples_match_registered_provider_shapes() -> anyhow::Result<()> {
    for relative_path in [
        ...
        assert!(matches!(
            source.compatibility(),
            transferia_delivery_contracts::semantics::EndpointDescriptor::Postgres(_)
        ));
        assert!(matches!(
            sink.compatibility(),
            transferia_delivery_contracts::semantics::EndpointDescriptor::ClickHouse
                | transferia_delivery_contracts::semantics::EndpointDescriptor::S3(_)
        ));
    }
    Ok(())
}
...
async fn ytsaurus_examples_match_registered_provider_shapes() -> anyhow::Result<()> {
    for relative_path in [
        ...
        let config = Config::from_file(workspace_root().join(relative_path))?;
        let (source, sink) = build_resolved_endpoints(&config).await?;
        if relative_path.contains("source") {
            assert!(matches!(
                source.compatibility(),
                transferia_delivery_contracts::semantics::EndpointDescriptor::YTsaurus(_)
            ));
        } else {
            assert!(matches!(
                sink.compatibility(),
                transferia_delivery_contracts::semantics::EndpointDescriptor::YTsaurusSink
            ));
        }
    }
    Ok(())
