...
impl<'a> BinlogStreamRequest<'a> {
    ...
    /// Limits one complete binlog event, excluding the one-byte replication
    /// packet status prefix. The limit is installed in the packet decoder
    /// before the first event packet is read, so oversized multi-chunk packets
    /// are rejected before their declared size is allocated.
