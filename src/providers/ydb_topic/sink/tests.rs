...
use crate::providers::ydb_topic::src_stream::YdbTopicDriver;

fn config(extra: &str) -> anyhow::Result<YdbTopicSinkConfig> {
...
fn accepts_ydb_and_pqv1_drivers() -> anyhow::Result<()> {
    let ydb = config("")?;
    ydb.validate()?;
    assert_eq!(ydb.driver, YdbTopicDriver::Ydb);

    let pqv1: YdbTopicSinkConfig = serde_yaml::from_str(
        "host: localhost\nport: 2135\ntopic_path: /demo/events\nproducer_id: transferia\npartition_id: 2\nauth: { type: token_file, token_file: /tmp/token }\ndriver: pqv1\ntrusted_plaintext: true\n",
    )?;
    pqv1.validate()?;
    assert_eq!(pqv1.driver, YdbTopicDriver::Pqv1);
    Ok(())
}
