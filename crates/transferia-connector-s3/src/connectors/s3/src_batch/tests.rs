...
#[tokio::test]
async fn data_sample_applies_configured_s3_json_parser_and_preserves_integer_precision(
) -> anyhow::Result<()> {
    ...
    store
        .put(
            &Path::from("events/a.json"),
            Bytes::from_static(b"{\"id\":9007199254740993}\n").into(),
        )
        .await?;
    let tables = super::preview::sample_data(
        &config,
        store,
        source.parser(),
        sample_limits(),
...
#[tokio::test]
async fn data_sample_rejects_an_oversized_object_instead_of_parsing_a_valid_prefix(
) -> anyhow::Result<()> {
    use transferia_registry::SourceConnector as _;
    let config = sample_json_config();
    let source = S3SourceConnector::from_config(config.clone(), Arc::new(MetricsRegistry::new()))?;
    ...
    store
        ...
        .await?;
    let result = super::preview::sample_data(
        &config,
        store,
        source.parser(),
        transferia_registry::TableSampleLimits {
...
async fn data_sample_reads_native_parquet_with_the_requested_row_limit() -> anyhow::Result<()> {
    use transferia_registry::SourceConnector as _;
    let config: S3SourceConfig = serde_yaml::from_str(
    ...
    let store = Arc::new(InMemory::new());
    store
        .put(&Path::from("first.parquet"), Bytes::from(bytes).into())
        .await?;
    let tables = super::preview::sample_data(
        &config,
        store,
...
async fn data_sample_rejects_parquet_expansion_before_decoding_rows() -> anyhow::Result<()> {
    ...
    store
        .put(&Path::from("first.parquet"), Bytes::from(bytes).into())
        .await?;
    assert!(super::preview::sample_data(
        &config,
        store,
        source.parser(),
        limits,
