...
try {
  ...
  assert.equal(await page.getByRole("button", { name: "Show all", exact: true }).count(), 0, "three matches fit without a height toggle");
  ...
  assert.equal(await page.getByRole("button", { name: "Load tables", exact: true }).count(), 0, "preview reuses the source catalog");
