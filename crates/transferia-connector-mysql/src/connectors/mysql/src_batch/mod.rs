...
pub(crate) fn type_mapping() -> transferia_registry::type_mapping::TypeMapping {
    ...
    for name in [
        ...
        for unsigned in [false, true] {
            ...
            rows.push(TypeMappingRow::evaluate(
                format!("{name}{}", if unsigned { " unsigned" } else { "" }),
                connector::mysql_column_kind(name, unsigned, Some("utf8mb4")).map(|kind| {
                    format!("{:?} · {}", kind.arrow_type(), kind.arrow_extension_name())
                }),
            ));
        }
    }
    rows.push(TypeMappingRow::evaluate(
        ...
        connector::mysql_column_kind("varchar", false, Some("latin1"))
