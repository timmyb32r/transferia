...
// Ajv's ESM exports still reference CommonJS runtime helpers. Bundle those
// helpers into the artifact so it also loads directly as a browser module.
const validators = await build({
  stdin: { contents: standaloneCode(ajv, validatorExports), resolveDir: fileURLToPath(new URL("../", import.meta.url)) },
  bundle: true, write: false, format: "esm", platform: "browser", minify: true,
});
const validatorSource = validators.outputFiles[0].text;
...
// Exercise the generated ESM without a bundler/CommonJS shim on every freshness check.
await import(validatorsPath.href);
