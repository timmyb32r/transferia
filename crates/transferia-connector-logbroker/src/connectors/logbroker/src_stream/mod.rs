    ...
    fn sample_data(
        ...
        Box::pin(async move {
            ...
            let connection = LogbrokerSourceConnectionConfig {
                ...
                consumer_name: self.cfg.consumer_name.clone(),
                auth: self.cfg.auth.clone(),
                driver: self.cfg.driver,
                trusted_plaintext: self.cfg.trusted_plaintext,
                read_buffer_bytes: self.cfg.read_buffer_bytes,
            };
            let token = self.token().await?;
            let preview = transferia_connector_support::external_request::observe_external_request(
                "logbroker",
                "sample_read",
                source::preview_message(
                    &connection,
                    &token,
                    limits.max_bytes,
                    false,
            ...
            let message = transferia_core::data::message::Message {
                ...
                headers: metadata
                    .message_metadata
                    .into_iter()
                    .map(|item| transferia_core::data::message::MessageHeader {
                        key: Arc::from(item.key),
                        value: Some(bytes::Bytes::from(item.value)),
                    })
                    .collect::<Vec<_>>()
                    .into(),
                meta: transferia_core::data::message::MessageMeta {
                    topic: Some(Arc::from(metadata.topic)),
                    partition: Some(metadata.partition),
                    offset: Some(metadata.offset),
                    write_timestamp_ms: metadata.written_at_ms,
                },
            };
