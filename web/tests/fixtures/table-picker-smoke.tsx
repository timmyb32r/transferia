...
import { DataSchemaInspector } from "../../src/delivery/EditorViews";
...
import type { ConnectorDefinition, DiscoveryResult, JsonObject } from "../../src/types";
...
const discard = catalog.connectors.find(item => item.key === "discard")! as unknown as ConnectorDefinition;
const schema: DiscoveryResult = {
  ...
  datasets: [{ role: "Main", name: "CHARACTER_SETS", intermediate_columns: [], final_columns:
    ["character_set_name", "CHARACTER_SET_NAME", "an_especially_long_column_name_that_must_not_overlap_the_type", "_system_topic", "_system_partition", "_system_offset", "_system_message_index"].map((name, index) => ({
      name, arrow_type: index < 3 ? "transferia.clickhouse.source_type(LowCardinality(Nullable(String)))" : index === 3 ? "Utf8" : "Int64",
      destination_type: "String", primary_key: false, nullable: false, low_cardinality: false,
    })),
  }],
};
