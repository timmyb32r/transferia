...
fn dataset(name: &str) -> DiscoveredDataset {
    ...
    DiscoveredDataset {
        namespace: Some(Arc::from("schema.with.dot")),
        name: Arc::from(name),
...
fn data(name: &str) -> anyhow::Result<TableData> {
    ...
    let batch = RecordBatch::try_new(
        Arc::new(schema),
        vec![Arc::new(StringArray::from(vec!["original"]))],
    )?;
    Ok(
        TableData::new(Arc::from(name), false, batch, SystemColumns::default())
            .with_namespace(Arc::from("schema.with.dot")),
    )
}

#[tokio::test]
async fn exact_and_regex_project_the_same_identity_at_catalog_startup_and_runtime(
) -> anyhow::Result<()> {
    for middleware in [
        ...
        assert!(
            Arc::ptr_eq(result.batch.column(0), batch.column(0)),
            "rename must not copy or mutate data"
        );
        assert!(Arc::ptr_eq(&result.batch.schema(), &batch.schema()));
        assert!(projected.stored_schema.columns[0].primary_key);
        assert_eq!(
            projected.stored_schema.columns[0]
                .arrow_extension_metadata
                .as_deref(),
            Some("exact metadata")
        );
        assert_eq!(
            projected.incoming_schema.columns[0].arrow_extension_name,
            Some("test.extension")
        );
    }
    Ok(())
    ...
    for (pattern, replacement, input, expected) in [
        ...
        assert_eq!(
            &*regex(pattern, replacement)?
                .output_table_identity(None, input)?
                .1,
            expected
        );
    }
...
fn invalid_configuration_fails_before_discovery() {
    ...
    for (pattern, replacement) in [
        ...
        assert!(
            regex(pattern, replacement).is_err(),
            "{pattern:?} / {replacement:?}"
        );
    }
}
...
fn registration_exposes_both_typed_modes_and_rejects_mixed_or_unknown_fields() -> anyhow::Result<()>
{
    ...
    for yaml in [
        ...
        assert!(registry
            .build_middleware("rename_table", serde_yaml::from_str(yaml)?)
            .is_err());
    }
    Ok(())
}
...
#[tokio::test]
async fn full_identity_and_last_part_have_identical_catalog_startup_runtime_semantics(
) -> anyhow::Result<()> {
    for (config, expected_namespace, expected_name) in [
        (
            serde_json::json!({"mode":"regex", "pattern":r"^information_schema\.(.*)$", "replacement":"archive.${1}2"}),
            Some("archive"),
            "sql_features2",
        ),
        (
            serde_json::json!({"mode":"regex", "pattern":"^(.*)$", "replacement":"${1}2", "last_part_only":true}),
            Some("information_schema"),
            "sql_features2",
        ),
        (
            serde_json::json!({"mode":"exact", "name":"archive.events"}),
            Some("archive"),
            "events",
        ),
        (
            serde_json::json!({"mode":"exact", "name":"events"}),
            None,
            "events",
        ),
        (
            serde_json::json!({"mode":"exact", "name":"archive.events", "last_part_only":true}),
            Some("information_schema"),
            "archive.events",
        ),
    ] {
...
#[tokio::test]
async fn full_name_regex_preserves_escaped_identifiers_and_rejects_invalid_outputs(
) -> anyhow::Result<()> {
    let middleware = RenameTableMiddleware::new(RenameTableConfig::Regex {
        pattern: "^(.*)$".into(),
        replacement: "${1}2".into(),
        last_part_only: false,
    })?;
    ...
    for replacement in [
        ...
        assert!(
            middleware.output_dataset(&dataset("events")).await.is_err(),
            "{replacement:?}"
        );
        assert!(
            middleware.process(data("events")?).await.is_err(),
            "{replacement:?}"
        );
        assert!(middleware
            .validate_preview_identity(Some("public"), "events")
            .is_err());
    }
    Ok(())
}
...
fn preview_requires_regex_match_in_the_explicitly_selected_name_scope() -> anyhow::Result<()> {
    for last_part_only in [false, true] {
        let middleware = RenameTableMiddleware::new(RenameTableConfig::Regex {
            pattern: if last_part_only {
                "^events$"
            } else {
                r"^public\.events$"
            }
            .into(),
            replacement: "${0}2".into(),
            last_part_only,
        })?;
        middleware.validate_preview_identity(Some("public"), "events")?;
        let error = middleware
            .validate_preview_identity(Some("public"), "other")
            .unwrap_err()
            .to_string();
