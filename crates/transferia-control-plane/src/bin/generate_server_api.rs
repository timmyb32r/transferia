...
fn main() -> anyhow::Result<()> {
    let artifacts = [
        (
            SCHEMA_OUTPUT,
            transferia_control_plane::server::api_contract::schema()?,
        ),
        (
            FIXTURE_OUTPUT,
            transferia_control_plane::server::api_contract::fixture()?,
        ),
    ];
    if std::env::args().any(|argument| argument == "--check") {
        for (output, generated) in artifacts {
            ...
            anyhow::ensure!(
                committed == generated,
                "{output} is stale; run `TRANSFERIA_SKIP_SERVER_UI=1 cargo run -p transferia-control-plane --bin generate-server-api`"
            );
        }
        return Ok(());
    }
