...
use mysql_common::{
    binlog::{
        consts::{BinlogChecksumAlg, BinlogVersion::Version4},
        events::{BinlogEventFooter, BinlogEventHeader, Event, TableMapEvent},
        EventStreamReader,
    },
    io::ParseBuf,
...
impl super::Conn {
    ...
    pub async fn get_binlog_stream(
        ...
        let current_packet_limit = self.stream_ref()?.get_max_allowed_packet();
        let max_event_bytes = request
            .max_event_bytes
            .unwrap_or_else(|| current_packet_limit.saturating_sub(1));
        let packet_limit = binlog_packet_limit(max_event_bytes)?;
        self.request_binlog(request).await?;
        self.stream_mut()?.set_max_allowed_packet(packet_limit);

        Ok(BinlogStream::new(self, max_event_bytes))
    }
...
/// Transaction payload events are yielded as received and are not automatically decompressed.
pub struct BinlogStream {
    read_packet: ReadPacket<'static, 'static>,
    esr: EventStreamReader,
    format_description_event: Option<Vec<u8>>,
    max_event_bytes: usize,
}

impl BinlogStream {
    /// `conn` is a `Conn` with `request_binlog` executed on it.
    pub(super) fn new(conn: Conn, max_event_bytes: usize) -> Self {
        BinlogStream {
            read_packet: ReadPacket::new(conn),
            esr: EventStreamReader::new(Version4),
            format_description_event: None,
            max_event_bytes,
        }
    }
    ...
    /// Evicts all retained table maps while preserving the active format description.
    ///
    /// Callers must invoke this only after they have completely decoded a transaction-ending
    /// XID/COMMIT/ROLLBACK, or at an FDE/Rotate boundary. Row events depend on the preceding
    /// table-map event and clearing earlier would make an otherwise valid transaction undecodable.
    pub fn clear_table_maps(&mut self) -> Result<()> {
        self.esr = event_reader_without_table_maps(self.format_description_event.as_deref())?;
        Ok(())
    }
...
impl futures_core::stream::Stream for BinlogStream {
    ...
    fn poll_next(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        let packet = match ready!(Pin::new(&mut self.read_packet).poll(cx)) {
            Ok(packet) => packet,
            Err(error) => {
                return Poll::Ready(Some(Err(map_binlog_packet_error(
                    error,
        ...
        if first_byte == Some(0) {
            ...
            if let Err(error) = validate_binlog_event_size(event_data, self.max_event_bytes) {
                return Poll::Ready(Some(Err(error.into())));
            }
            if let Err(error) = validate_binlog_event_reader_safety(event_data, &self.esr) {
                return Poll::Ready(Some(Err(error.into())));
            }
            match self.esr.read(event_data) {
                Ok(Some(event)) => {
                    if event.header().event_type_raw()
                        == mysql_common::binlog::consts::EventType::FORMAT_DESCRIPTION_EVENT as u8
                    {
                        self.format_description_event = Some(event_data.to_vec());
                    }
            ...
            Poll::Ready(Some(Err(unexpected_binlog_packet(
                first_byte,
                packet.len(),
            )
            .into())))
...
fn unexpected_binlog_packet(first_byte: Option<u8>, packet_bytes: usize) -> DriverError {
...
fn event_reader_without_table_maps(
    format_description_event: Option<&[u8]>,
) -> Result<EventStreamReader> {
    let mut reader = EventStreamReader::new(Version4);
    if let Some(event) = format_description_event {
        let restored = reader.read(event)?;
        if restored.as_ref().is_none_or(|event| {
            event.header().event_type_raw()
                != mysql_common::binlog::consts::EventType::FORMAT_DESCRIPTION_EVENT as u8
        }) {
            return Err(std::io::Error::other(
                "saved binlog format-description event could not be restored",
            )
            .into());
        }
    }
...
const MAX_BINLOG_EVENT_BYTES: usize = if usize::BITS > 32 {
    u32::MAX as usize
} else {
    usize::MAX - 1
};

fn binlog_packet_limit(max_event_bytes: usize) -> Result<usize> {
    if !(BinlogEventHeader::LEN..=MAX_BINLOG_EVENT_BYTES).contains(&max_event_bytes) {
        return Err(DriverError::InvalidBinlogEventSizeLimit {
            max_event_bytes,
            minimum_event_bytes: BinlogEventHeader::LEN,
            maximum_event_bytes: MAX_BINLOG_EVENT_BYTES,
        }
        .into());
    }
    Ok(max_event_bytes + 1)
}

fn validate_binlog_event_size(
    event_data: &[u8],
    max_event_bytes: usize,
) -> std::result::Result<(), DriverError> {
    if event_data.len() > max_event_bytes {
        return Err(DriverError::BinlogEventTooLarge { max_event_bytes });
    }
    if event_data.len() < BinlogEventHeader::LEN {
        return Err(DriverError::TruncatedBinlogEventHeader {
            actual_event_bytes: event_data.len(),
            minimum_event_bytes: BinlogEventHeader::LEN,
        });
    }
    let declared_event_bytes = u32::from_le_bytes([
        event_data[9],
        event_data[10],
        event_data[11],
        event_data[12],
    ]);
    if declared_event_bytes as usize != event_data.len() {
        return Err(DriverError::InvalidBinlogEventSize {
...
fn validate_binlog_event_reader_safety(
    event_data: &[u8],
    reader: &EventStreamReader,
) -> std::result::Result<(), DriverError> {
    let footer = reader.get_fde().footer();
    let checksum_suffix_required = footer.get_checksum_enabled()
        && !matches!(
            footer.get_checksum_alg(),
            Ok(None | Some(BinlogChecksumAlg::BINLOG_CHECKSUM_ALG_OFF))
        );
    let minimum_event_bytes = BinlogEventHeader::LEN + BinlogEventFooter::BINLOG_CHECKSUM_LEN;
    if checksum_suffix_required && event_data.len() < minimum_event_bytes {
        return Err(DriverError::TruncatedChecksummedBinlogEvent {
            actual_event_bytes: event_data.len(),
            minimum_event_bytes,
        });
    }
...
fn is_packet_too_large(error: &IoError) -> bool {
    match error {
        IoError::Io(error) => error
            .get_ref()
            .and_then(|source| source.downcast_ref::<PacketCodecError>())
            .is_some_and(|source| matches!(source, PacketCodecError::PacketTooLarge)),
        #[cfg(any(feature = "native-tls-tls", feature = "rustls-tls"))]
        IoError::Tls(_) => false,
    }
}

fn map_binlog_packet_error(error: IoError, max_event_bytes: usize) -> Error {
    if is_packet_too_large(&error) {
        DriverError::BinlogEventTooLarge { max_event_bytes }.into()
    } else {
        error.into()
    }
}
...
#[cfg(test)]
mod raw_transaction_payload_tests;
