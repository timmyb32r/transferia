...
fn plugin_detection_does_not_hide_permissions_or_missing_wal() {
    assert!(plugin_is_missing(&SqlState::UNDEFINED_FILE, "could not access file \"wal2json\": No such file or directory", "wal2json"));
    assert!(!plugin_is_missing(&SqlState::UNDEFINED_FILE, "could not open file pg_wal/0001", "wal2json"));
    assert!(!plugin_is_missing(&SqlState::INSUFFICIENT_PRIVILEGE, "permission denied", "pgoutput"));
    assert!(!plugin_is_missing(&SqlState::UNDEFINED_FILE, "could not access file \"wal2json\": Permission denied", "wal2json"));
    assert!(!plugin_is_missing(&SqlState::UNDEFINED_FILE, "could not access file \"pgoutput\": No such file or directory", "wal2json"));
}
...
fn resuming_auto_never_changes_the_existing_slots_plugin() {
    assert_eq!(select_auto_plugin(true, true, Some("wal2json")).unwrap(), "wal2json");
    assert_eq!(select_auto_plugin(true, true, Some("pgoutput")).unwrap(), "pgoutput");
...
fn plugin_is_advanced_defaults_to_auto_and_legacy_settings_are_rejected() {
    ...
    for invalid in [r#"{"slot":"dtt123"}"#, r#"{"decoder":{"type":"wal2_json"}}"#, r#"{"plugin":{"type":"wal2_json"}}"#] {
    ...
    assert_eq!(properties["plugin"]["default"], serde_json::json!({"type": "auto"}));
    assert_eq!(properties["plugin"]["x-ui"]["section"], "advanced");
    let branches = schema["$defs"]["ReplicationPlugin"]["oneOf"].as_array().unwrap();
    let tags = branches.iter().map(|branch| branch["properties"]["type"]["const"].as_str().unwrap()).collect::<Vec<_>>();
