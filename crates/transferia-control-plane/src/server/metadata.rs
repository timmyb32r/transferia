...
impl MetadataSession {
    ...
    fn prefetch(self: &Arc<Self>, tasks: &tokio_util::task::TaskTracker) {
        if self.catalog.len() >= AUTOMATIC_SCHEMA_CATALOG_BOUNDARY {
            return;
        }
    ...
    pub(super) fn selected_for_preview(
        ...
        anyhow::ensure!(
            source.connector == self.connector
    ...
    pub(super) async fn sample(
        ...
        anyhow::ensure!(
            matches!(
                self.entries.get(&table).and_then(SchemaEntry::get),
                Some(Ok(()))
            ),
    ...
    fn selected(&self, config: &Value) -> anyhow::Result<Vec<TableIdentity>> {
        ...
        let visible = self
            .catalog
            .iter()
            .filter(|table| self.reader.includes_table(table, hide))
            .cloned()
            .collect::<Vec<_>>();
    ...
    async fn ensure_tables(&self, tables: &[TableIdentity]) -> anyhow::Result<()> {
        ...
        for table in &tables {
            anyhow::ensure!(
                ...
                "Table {} is not in the authenticated catalog; refresh metadata",
                table.qualified_name()
            );
        }
        let pending = tables
            .iter()
            .filter(|table| self.entries[*table].get().is_none())
            .cloned()
            .collect::<Vec<_>>();
        for chunk in pending.chunks(SCHEMA_BATCH_SIZE) {
            ...
            let _gate = self
                .run(&self.cancellation, async {
                    Ok(self.load_gate.lock().await)
                })
                .await?;
            let missing = chunk
                .iter()
            ...
            let result = self
                .reader
                .load_tables(missing.clone(), self.cancellation.child_token())
                .await;
            self.ensure_active()?;
            let result = result.and_then(|results| {
                anyhow::ensure!(
                    results.len() == missing.len()
                        && missing.iter().all(|table| results.contains_key(table)),
            ...
            let results = match result {
                ...
                Err(error) => {
                    ...
                    missing
                        .into_iter()
                        .map(|table| (table, Err(message.clone())))
                        .collect()
                }
            };
            for (table, result) in results {
                self.entries[&table]
                    .set(result)
                    .map_err(|_| anyhow::anyhow!("Metadata entry initialized twice"))?;
            }
        }
        self.ensure_active()?;
        for table in &tables {
    ...
    async fn status(&self) -> MetadataStatus {
        ...
        for (table, entry) in &self.entries {
            match entry.get() {
                ...
                Some(Err(message)) => errors.push(TableMetadataError {
                    table: table.clone(),
        ...
        MetadataStatus {
            ...
            loaded,
            errors,
            loading: self.active_loads.load(Ordering::Relaxed) > 0,
            validation,
        }
    }

    pub(super) async fn begin_validation(
        &self,
        id: &str,
        revision: u64,
        config: &Value,
        ...
        if kind != self.connector || metadata_identity(source) != self.identity {
            return Err(ServiceError::Validation(
                "Source changed; refresh metadata before validation".into(),
            ));
        }
        let selected = self
            .selected(source)
            .map_err(|error| ServiceError::Validation(format!("{error:#}")))?;
        *self.validation.lock().await = Some((
            MetadataValidationProgress {
    ...
    pub(super) async fn finish_validation(&self, id: &str, revision: u64, success: bool) {
        if let Some((progress, _)) = self.validation.lock().await.as_mut() {
            if progress.delivery_id == id && progress.revision == revision {
                progress.phase = if success {
                    MetadataValidationPhase::Complete
                } else {
                    MetadataValidationPhase::Failed
                };
            }
        }
    }
...
fn metadata_scan_config(config: &Value) -> anyhow::Result<Value> {
    ...
    let object = config
        .as_object_mut()
        .ok_or_else(|| anyhow::anyhow!("Invalid source configuration"))?;
...
fn configured_source(config: &Value) -> anyhow::Result<(&str, &Value)> {
    ...
    let (kind, source) = source
        .iter()
        .next()
        .ok_or_else(|| anyhow::anyhow!("Choose a source"))?;
    Ok((kind, source))
}
...
impl CachedDiscovery {
    pub(super) fn new(session: Arc<MetadataSession>, config: &Value) -> anyhow::Result<Self> {
        ...
        anyhow::ensure!(
            session.connector == kind && session.identity == metadata_identity(source),
...
impl PreviewDiscoveryProvider for CachedDiscovery {
    fn discover<'a>(
        &'a self,
        kind: &'a str,
        resolved: &'a serde_yaml::Value,
        context: SourceDiscoveryContext,
    ) -> std::pin::Pin<
        Box<dyn std::future::Future<Output = anyhow::Result<DeliveryDiscovery>> + Send + 'a>,
    > {
        Box::pin(async move {
            anyhow::ensure!(
                kind == self.session.connector
                    && context.delivery_type == self.session.delivery_type,
                "Source mode changed; discover tables again"
            );
            anyhow::ensure!(
                metadata_identity(&serde_json::to_value(resolved)?)
                    == self.session.resolved_identity,
            ...
            self.session
                .run(
                    ...
                    self.session.ensure_tables(&self.selected),
                )
                .await?;
...
impl ControlPlane {
    pub async fn connect_metadata(
        &self,
        request: MetadataConnectRequest,
        cancellation: CancellationToken,
        ...
        let (mut connection, resolved) = self
            .check_connection_resolved(
        ...
        if !matches!(
            ...
            return Err(ServiceError::Validation(
                "Authenticate the source to load metadata".to_owned(),
            ));
        }
        if resolved.len() != 1 {
            return Err(ServiceError::Validation(
                "Metadata requires one resolved source endpoint".to_owned(),
            ));
        }
        ...
        let catalog = self.transferia.build_registry(&Arc::new(
            transferia_connectors::metrics::MetricsRegistry::new(),
        ))?;
        let source = catalog.build_source(
            &request.source.connector,
            serde_yaml::to_value(raw).map_err(anyhow::Error::from)?,
        )?;
        if !source
            .compatibility(request.delivery_type)
            .supports_delivery_type(request.delivery_type)
        {
            return Err(ServiceError::Validation(
                "Source does not support the requested delivery type".into(),
            ));
        }
        let reader = source
            .metadata_reader(request.delivery_type)?
            .ok_or_else(|| {
        ...
        let tables = reader
            .list_tables(cancellation.clone())
            .await
        ...
        Ok(MetadataConnection {
            connection,
            metadata: session.status().await,
        })
    }

    pub(super) async fn metadata_session(
        &self,
        id: &str,
    ) -> Result<Arc<MetadataSession>, ServiceError> {
        self.metadata_sessions
            .lock()
            .await
            .get(id)
    ...
    pub async fn release_metadata(&self, id: &str) -> Result<MetadataStatus, ServiceError> {
        ...
        let session = self
            ...
            .remove(id)
            .ok_or_else(|| {
                ServiceError::NotFound("Metadata cache is no longer available".to_owned())
            })?;
        session.cancellation.cancel();
        Ok(session.status().await)
    }

    pub async fn load_metadata_schemas(
        &self,
        id: &str,
        request: MetadataSchemasRequest,
        cancellation: CancellationToken,
    ) -> Result<MetadataStatus, ServiceError> {
        let session = self.metadata_session(id).await?;
        session.ensure_active()?;
        if session.connector != request.source.connector
            || session.identity != metadata_identity(&request.source.config)
        {
            return Err(ServiceError::Validation(
                "Source changed; discover tables again".to_owned(),
            ));
        }
        let selected: BTreeSet<_> = session
            .selected(&request.source.config)?
            .into_iter()
            .collect();
        for table in &request.tables {
            if !selected.contains(table) {
                return Err(ServiceError::Validation(format!(
                    "Table {} is not selected by the source",
                    table.qualified_name()
                )));
            }
        }
        ...
        session
            .run(&cancellation, session.ensure_tables(&request.tables))
            .await
            .map_err(|error| ServiceError::Validation(format!("{error:#}")))?;
        drop(_loading);
        Ok(session.status().await)
    ...
    pub async fn cached_source_discovery(
        &self,
        id: &str,
        config: &Value,
        cancellation: CancellationToken,
        ...
        let mode: DeliveryType =
            serde_json::from_value(config.get("delivery_type").cloned().unwrap_or(Value::Null))
                .map_err(anyhow::Error::from)?;
        if mode != session.delivery_type {
            return Err(ServiceError::Validation(
                "Source mode changed; refresh metadata".to_owned(),
            ));
        }
        ...
        for table in &provider.selected {
            match session.entries.get(table).and_then(SchemaEntry::get) {
                ...
                None => {
                    return Err(ServiceError::Validation(
                        "Load the selected table schemas or run Validate".to_owned(),
                    ))
                }
            }
        }
