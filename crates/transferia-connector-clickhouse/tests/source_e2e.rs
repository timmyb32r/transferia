...
use arrow::array::{
    Array as _, BinaryArray, Decimal256Array, Int16Array, Int64Array, ListArray, StringArray,
...
async fn native_table_preview_is_bounded_lossless_with_quoted_names() -> anyhow::Result<()> {
    ...
    execute_fixture_query(
        &http,
        &url,
        "CREATE USER sample_reader IDENTIFIED WITH plaintext_password BY 'read-only-test'",
    )
    .await?;
    execute_fixture_query(
        &http,
        &url,
        "GRANT SELECT ON default.`events\\`quoted` TO sample_reader",
    )
    .await?;
    ...
    assert_eq!(
        sample.batch.column(4).data_type(),
    ...
    let count = http
        .post(&url)
        .body("SELECT count(*) FROM default.`events\\`quoted`")
        .send()
        .await?
        .error_for_status()?
        .text()
    ...
    execute_fixture_query(
        &http,
        &url,
        "CREATE TABLE default.wide_sample (payload String) ENGINE=Memory",
    )
    .await?;
    execute_fixture_query(
        &http,
        &url,
        "INSERT INTO default.wide_sample VALUES (repeat('a',65536))",
    )
    .await?;
    execute_fixture_query(
        &http,
        &url,
        "GRANT SELECT ON default.wide_sample TO sample_reader",
    )
    .await?;
    let error = registry
        .sample_source_table(
...
async fn type_fixture() -> anyhow::Result<(
...
async fn execute_fixture_query(
    http: &reqwest::Client,
    url: &str,
    query: &str,
) -> anyhow::Result<()> {
    ...
    anyhow::ensure!(
        status.is_success(),
        "ClickHouse fixture setup failed: {status}: {body}"
    );
    Ok(())
}
...
) -> anyhow::Result<transferia_core::delivery::DeliveryDiscovery> {
    connector
...
async fn build_type_snapshot(
    connector: &ClickHouseSourceConnector,
) -> anyhow::Result<Box<dyn transferia_core::source::Source>> {
    connector
...
async fn quoted_source_identifiers_are_preserved_by_both_readers() -> anyhow::Result<()> {
    let quote_identifier =
        |name: &str| format!("`{}`", name.replace('\\', "\\\\").replace('`', "\\`"));
    let (_container, host, native_port, http_port) = type_fixture().await?;
    let http = reqwest::Client::new();
    let url = format!("http://{host}:{http_port}");
    let names = [
    ...
    let declarations = names
        .iter()
        .map(|name| format!("{} Int64", quote_identifier(name)))
        .collect::<Vec<_>>()
        .join(", ");
    execute_fixture_query(
        &http,
        &url,
        &format!(
            "CREATE TABLE quoted_events ({declarations}) ENGINE=MergeTree ORDER BY {}",
            quote_identifier(names[1]),
        ),
    )
    .await?;
    execute_fixture_query(
        &http,
        &url,
        "INSERT INTO quoted_events VALUES (100,101,102,103,104,105,106,107,108)",
    )
    .await?;
    ...
    execute_fixture_query(
        &http,
        &url,
        "INSERT INTO nested_events VALUES (1, ['q1','q2'], [2,3])",
    )
    .await?;
    for reader in ["native", "parquet"] {
        ...
        assert!(discovery.datasets[0]
            ...
            .columns
            .iter()
            .any(|column| column.name == "entries.query_id"));
        let batch = read_type_snapshot(&connector).await?;
        let entries = batch
            .column_by_name("entries.query_id")
            .unwrap()
            .as_any()
        ...
        assert_eq!(
            ...
            vec![Some(b"q1".as_slice()), Some(b"q2".as_slice())]
        );
    }
    Ok(())
}
...
async fn readable_generated_columns_work_with_both_source_readers() -> anyhow::Result<()> {
    ...
    for reader in ["native", "parquet"] {
        ...
        let discovery = discover_types(&connector).await?;
        assert_eq!(
            discovery.datasets[0].stored_schema.columns.len(),
            4,
            "{reader}"
        );
        let batch = read_type_snapshot(&connector).await?;
        ...
        let computed = batch
            ...
            .unwrap();
        let displayed = batch
            .column_by_name("displayed")
            .unwrap()
            .as_any()
            .downcast_ref::<Int64Array>()
            .unwrap();
        let databases = batch
            .column_by_name("databases")
            .unwrap()
            .as_any()
            .downcast_ref::<ListArray>()
            .unwrap();
        ...
        for row in 0..batch.num_rows() {
            ...
            let values = values.as_any().downcast_ref::<BinaryArray>().unwrap();
            let expected: Vec<&[u8]> = if id == 1 {
                vec![b"db1"]
            } else {
                vec![b"explicit", b"db2"]
            };
            assert_eq!(
                values.iter().collect::<Vec<_>>(),
...
async fn both_source_readers_preserve_full_enum_and_nested_numeric_types() -> anyhow::Result<()> {
    ...
    let http = reqwest::Client::new();
    let url = format!("http://{host}:{http_port}");
    let label = |code: i16| match code {
        0 => "NO".to_owned(),
        1 => "YES".to_owned(),
        _ => format!("label_{code}"),
    };
    let labels = (-128_i16..=127)
        .map(|code| format!("'{}' = {code}", label(code)))
        .collect::<Vec<_>>()
        .join(", ");
    ...
    for reader in ["native", "parquet"] {
        ...
        let expected_amount = arrow::datatypes::i256::from_string(
        ...
        for row in 0..256 {
            ...
            assert_eq!(
                statuses.value(row),
                label(i16::try_from(id)? - 128),
                "{reader}"
            );
            assert_eq!(
                wide.value(row),
                if id % 2 == 0 { "negative" } else { "positive" }
            );
            ...
            assert_eq!(
                times.value(row),
                if id % 2 == 0 { 1_234_560 } else { -1_234_560 }
            );
        }
...
#[tokio::test]
async fn unsupported_dynamic_type_defaults_to_string_and_allows_explicit_fail_for_both_readers(
) -> anyhow::Result<()> {
    let (_container, host, native_port, http_port) = type_fixture().await?;
    let http = reqwest::Client::new();
    let url = format!("http://{host}:{http_port}");
    ...
    for reader in ["native", "parquet"] {
        let strict = type_connector(
            &host,
            native_port,
            http_port,
            reader,
            "unsupported_events",
            "unsupported_types: fail\n",
        )?;
        ...
        assert!(
            message.contains("payload")
                ...
                && message.contains("to_string"),
            "{message}"
        );
        let converted = type_connector(
            &host,
            native_port,
            http_port,
            reader,
        ...
        for row in 0..batch.num_rows() {
            ...
            assert!(
                actual.insert(id, payload).is_none(),
                "{reader}: duplicate id {id}"
            );
        }
        assert_eq!(
            actual,
            BTreeMap::from([
                (1, Some("18446744073709551615")),
                (2, Some("unchanged text")),
                (3, None),
            ]),
            "{reader}"
        );
...
#[tokio::test]
async fn changed_enum_declaration_after_discovery_fails_before_any_source_batch(
) -> anyhow::Result<()> {
    let (_container, host, native_port, http_port) = type_fixture().await?;
    let http = reqwest::Client::new();
    let url = format!("http://{host}:{http_port}");
    ...
    execute_fixture_query(
        &http,
        &url,
        "ALTER TABLE drift_events MODIFY COLUMN status Enum8('a' = 1, 'b' = 2, 'c' = 3)",
    )
    .await?;
...
async fn clickhouse_source_discovers_and_streams_a_deterministic_native_snapshot(
    ...
    let connector =
        ClickHouseSourceConnector::from_config(config.clone(), Arc::new(MetricsRegistry::new()))?;
