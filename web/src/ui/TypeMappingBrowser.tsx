...
export function TypeMappingBrowser({ catalog, role }: {
  ...
  const transport = ["kafka", "logbroker", "s3"].includes(selected?.key ?? "");
  const showUnsupported = unsupportedOnly && !!mapping?.rows.length;
  const explanation = transport && !showUnsupported
    ? role === "source"
  ...
  const rejectedCount = mapping?.rows.filter((row) => row.error != null).length ?? 0;
  const rows = mapping?.rows.filter((row) => (!showUnsupported || row.error != null) && [row.input, row.output, row.error].some((text) => text?.toLocaleLowerCase().includes(query))) ?? [];
  return <div class="type-mapping-browser">
    <nav class="type-mapping-connectors" aria-label={role === "source" ? "Sources" : "Destinations"}>
      ...
      <div class="type-mapping-connector-list">
        {connectors.filter((c) => c.title.toLocaleLowerCase().includes(connectorSearch.trim().toLocaleLowerCase())).map((c) =>
          <Button key={c.key} variant="plain" aria-pressed={selected?.key === c.key} onClick={() => { setConnectorKey(c.key); setTypeSearch(""); setUnsupportedOnly(false); }}>{c.title}</Button>)}
      </div>
    </nav>
    <section class={`type-mapping-detail${explanation ? " type-mapping-detail-message" : ""}`} aria-label="Type mappings">
      ...
      <div class="type-mapping-tools">
        {!explanation ? <AutofillResistantInput type="search" aria-label="Find type" placeholder="Find a type or rejection reason" value={typeSearch} onInput={(e) => setTypeSearch(e.currentTarget.value)} /> : <span />}
        {mapping?.rows.length && selected?.key !== "discard" && selected?.key !== "data_generator" && (!transport || role === "sink") ? <Button aria-pressed={showUnsupported} onClick={() => setUnsupportedOnly(!showUnsupported)} title="Rejections evaluated by the production resolver for the concrete types shown; other parameters and formats may differ.">
          {transport ? "Unsupported types · JSON" : "Unsupported types"} ({rejectedCount})
        </Button> : null}
      </div>
      ...
      <div class="type-mapping-table-scroll" tabIndex={0} aria-label="Mapping examples">
        ...
        {rows.length === 0 && <p class="type-mapping-empty">{showUnsupported && !query ? "No rejections for the evaluated examples." : mapping?.rows.length ? "No matching types." : "No native type examples for this endpoint."}</p>}
      </div>
      </>}
    </section>
  </div>;
