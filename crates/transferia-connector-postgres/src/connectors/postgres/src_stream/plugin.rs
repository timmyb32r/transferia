...
pub async fn resolve_plugin(
    ...
    anyhow::ensure!(
        existing
            .as_deref()
            .is_none_or(|plugin| plugin == decoder.plugin()),
...
fn select_auto_plugin(
    ...
    match (existing, pgoutput, wal2json) {
        (Some("pgoutput") | None, true, _) => Ok("pgoutput"),
        ...
        (None, false, false) => {
            anyhow::bail!("Neither pgoutput nor wal2json is available on the PostgreSQL server")
        }
    }
}
...
async fn probe_plugin(client: &Client, plugin: &str) -> anyhow::Result<bool> {
    ...
    match result {
        ...
        Err(error)
            if error
                .as_db_error()
                .is_some_and(|error| plugin_is_missing(error.code(), error.message(), plugin)) =>
        {
            Ok(false)
        }
...
async fn ensure_auto_publication(
    ...
    anyhow::ensure!(
        !tables.is_empty(),
        "An automatic publication requires selected tables"
    );
    let marker = format!("transferia:{resource_key}");
    let transaction =
        observe_external_request("postgres", "begin_auto_publication", client.transaction())
            .await?;
    let existing = observe_external_request(
        "postgres",
    ...
    match existing {
        ...
        None => {
            ...
            let statement = format!(
                "CREATE PUBLICATION {} FOR TABLE {names}",
                quote_identifier(publication)
            );
            observe_external_request(
                "postgres",
                "create_auto_publication",
    ...
    let published = observe_external_request(
        ...
        transaction.query(
            "SELECT schemaname, tablename FROM pg_catalog.pg_publication_tables WHERE pubname = $1",
            &[&publication],
        ),
    )
    .await?;
    ...
    let expected = tables
        .iter()
        .map(|table| (table.config.schema.clone(), table.config.name.clone()))
        .collect::<std::collections::BTreeSet<_>>();
    anyhow::ensure!(
        actual == expected,
        "Automatic publication '{publication}' has a different table set; refusing to change it"
    );
