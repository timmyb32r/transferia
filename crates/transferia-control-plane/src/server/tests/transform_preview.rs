...
use transferia_registry::{
    ComponentRegistration, ConnectorDefinition, DeliveryMode, MiddlewareRegistration, Registry,
    RegistryBuilder,
};
...
impl Composition for SampleComposition {
    ...
    fn build_registry(
        &self,
        metrics: &Arc<transferia_delivery_contracts::metrics::MetricsRegistry>,
    ) -> anyhow::Result<Registry> {
        let mut builder = RegistryBuilder::new();
        // Exercise real middleware through the production composition boundary,
        // while keeping the test source isolated from production connectors.
        let middleware_registry = Arc::new(Transferia::public()?.build_registry(metrics)?);
        for definition in middleware_registry.middleware_definitions() {
            let key = definition.key;
            let registry = middleware_registry.clone();
            builder.register_middleware(MiddlewareRegistration::new::<Value, _, _>(
                key,
                definition.title,
                || definition.initial.clone(),
                move |config| registry.build_middleware(key, serde_yaml::to_value(config)?),
            )?)?;
        }
        let reads = self.reads.clone();
        builder.register(
            ComponentRegistration::new("sample", "Sample")
                .source::<BTreeMap<String, Value>, _, _>(
                    vec![DeliveryMode::Batch],
                    false,
                    || serde_json::json!({}),
                    |_| anyhow::bail!("preview must not construct a delivery source"),
                )?
                .source_table_sampler::<Value, _, _>(move |_, table, limits, _| {
                    ...
                    async move {
                        ...
                        anyhow::ensure!(
                            ...
                            "explicit sample limits were not forwarded"
                        );
                        let batch = RecordBatch::try_new(
                                ...
                                Field::new("id", DataType::Int64, false).with_metadata(
                                    std::collections::HashMap::from([
                                        (
                                            ...
                                            "test.exact.integer".into(),
                                        ),
                                        (
                                            "ARROW:extension:metadata".into(),
                                            "{\"semantic\":\"original\"}".into(),
                                        ),
                            ...
                            vec![
                                ...
                                Arc::new(StringArray::from(vec!["drop", "keep"])),
                            ],
                        )?;
                        Ok(TableData::new(
                            Arc::from(table.name),
...
#[tokio::test]
async fn source_preview_executes_real_filter_then_sql_and_never_constructs_workers(
) -> anyhow::Result<()> {
    let composition = SampleComposition::default();
    let preview = ControlPlane::preview_transforms_with(
        &composition,
        request(
            &serde_json::json!([
    ...
    .await?;
    assert_eq!(
        serde_json::to_value(preview.before.rows)?,
        serde_json::json!([{"id":"3","kind":"keep"}])
    );
    assert_eq!(
        serde_json::to_value(preview.after.rows)?,
        serde_json::json!([{"doubled":"6"}])
    );
...
#[tokio::test]
async fn source_preview_rejects_malformed_action_before_resolving_or_reading_source(
) -> anyhow::Result<()> {
    let composition = SampleComposition::default();
...
#[tokio::test]
async fn source_preview_rejects_invalid_steps_and_limits_before_source_io() -> anyhow::Result<()> {
    for (patch, message) in [
        (serde_json::json!({"through_step":1}), "step"),
        (
            serde_json::json!({"middlewares":[{"filter":{},"datafusion":{}}]}),
            "exactly one",
        ),
        (
            serde_json::json!({"middlewares":[{"tables":{"include":""},"datafusion":{"sql":"SELECT * FROM input"}}]}),
            "Include",
        ),
        ...
        let mut value = serde_json::json!({
            ...
            "row_limit":20,"max_sample_bytes":16_777_216,"memory_limit_bytes":268_435_456,"timeout_ms":30_000
        });
        value
            .as_object_mut()
            .unwrap()
        ...
        .await
        .expect_err("invalid request must fail before source I/O");
...
async fn source_preview_timeout_cancels_endpoint_resolution() -> anyhow::Result<()> {
    let composition = SampleComposition {
        stall_resolution: true,
        ..Default::default()
    };
    let mut request = request(
        &serde_json::json!([{"datafusion":{"sql":"SELECT * FROM input"}}]),
        0,
    )?;
...
async fn source_preview_explains_unavailable_synthetic_event_metadata() -> anyhow::Result<()> {
    ...
    assert!(error
        .to_string()
        .contains("synthetic transport and CDC metadata are not available"));
    assert!(error.to_string().contains("_system_offset"));
    Ok(())
}

#[tokio::test]
async fn source_preview_preserves_exact_native_arrow_metadata_without_inventing_flags(
) -> anyhow::Result<()> {
    let preview = ControlPlane::preview_transforms_with(
        &SampleComposition::default(),
        request(
...
#[tokio::test]
async fn source_preview_executes_required_column_and_sql_validation_on_native_input(
) -> anyhow::Result<()> {
    for action in [
        ...
        let error = ControlPlane::preview_transforms_with(
            &SampleComposition::default(),
            request(&serde_json::json!([action]), 0)?,
            CancellationToken::new(),
            None,
        )
        .await
