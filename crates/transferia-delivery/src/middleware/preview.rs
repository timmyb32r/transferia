...
pub async fn preview_chain(
    ...
    anyhow::ensure!(
        through_step < middlewares.len(),
        "preview step index is out of range"
    );
    ...
    anyhow::ensure!(
        input.batch.get_array_memory_size() <= context.memory_limit_bytes,
    ...
    for (index, middleware) in middlewares.iter().enumerate().take(through_step + 1) {
        ...
        let operation = async {
            ...
            anyhow::ensure!(
                !output.is_dlq,
                "transform unexpectedly changed the dataset role"
            );
            Ok(output)
        };
        input = tokio::select! {
...
/// Display is deliberately separate from data execution.
///
/// Every non-null cell is Arrow-formatted text, not a JSON number; exact integers, decimals and
...
pub fn display_rows(data: &TableData) -> anyhow::Result<Vec<BTreeMap<String, Option<String>>>> {
    ...
    for field in schema.fields() {
        anyhow::ensure!(
            ...
            "preview cannot display duplicate column name {:?}",
            field.name()
        );
    }
    ...
    (0..data.batch.num_rows())
        .map(|row| {
            let mut values = BTreeMap::new();
            for ((field, array), formatter) in schema
                .fields()
                .iter()
                ...
                .zip(&formatters)
            {
                let value = if array.is_null(row) {
                    ...
                    Some(formatter.value(row).try_to_string().with_context(|| {
                        format!("preview column {:?}, row {}", field.name(), row + 1)
                    })?)
                };
