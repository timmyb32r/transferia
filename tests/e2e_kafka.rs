...
async fn kafka_sink_source_and_offset_commit_use_a_real_broker() -> anyhow::Result<()> {
    ...
    let sink_provider = KafkaSinkProvider::from_config(sink_config)?;
    sink_provider.limits().validate_discovery(&discovery)?;
    let sink = sink_provider
    ...
    let source_provider = KafkaSourceProvider::from_config(
    ...
    let restarted = KafkaSourceProvider::from_config(
        source_config(&broker, topic, "transferia-e2e-group")?,
        Arc::new(MetricsRegistry::new()),
    )?;
