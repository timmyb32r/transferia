...
impl Error {
    ...
    /// Returns whether packet decoding stopped before allocating beyond the configured
    /// `max_allowed_packet` boundary.
    pub fn is_packet_too_large(&self) -> bool {
        match self {
            Error::Driver(DriverError::PacketTooLarge) => true,
            Error::Io(IoError::Io(error)) => error
                .get_ref()
...
pub enum DriverError {
    ...
    #[error(
        "Unexpected binlog stream packet (first byte {first_byte:?}, {packet_bytes} bytes)."
    )]
    UnexpectedBinlogPacket {
    ...
    #[error(
        "Configured binlog event limit {max_event_bytes} bytes is outside the protocol range {minimum_event_bytes}..={maximum_event_bytes}."
    )]
    InvalidBinlogEventSizeLimit {
        max_event_bytes: usize,
        minimum_event_bytes: usize,
        maximum_event_bytes: usize,
    },

    #[error("Binlog event exceeds the configured {max_event_bytes}-byte limit.")]
    BinlogEventTooLarge { max_event_bytes: usize },

    #[error(
        "Binlog event packet contains {actual_event_bytes} bytes, shorter than the {minimum_event_bytes}-byte event header."
    )]
    TruncatedBinlogEventHeader {
        actual_event_bytes: usize,
        minimum_event_bytes: usize,
    },

    #[error(
        "Checksummed binlog event contains {actual_event_bytes} bytes, shorter than the required {minimum_event_bytes}-byte header and CRC32 suffix."
    )]
    TruncatedChecksummedBinlogEvent {
        actual_event_bytes: usize,
        minimum_event_bytes: usize,
    },

    #[error(
        "Binlog event header declares {declared_event_bytes} bytes, but its packet contains {actual_event_bytes} bytes."
    )]
    InvalidBinlogEventSize {
        declared_event_bytes: u32,
        actual_event_bytes: usize,
    },
