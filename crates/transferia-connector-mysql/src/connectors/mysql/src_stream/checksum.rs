...
impl BinlogChecksumVerifier {
    ...
    pub fn verify(&mut self, event: &Event) -> Result<(), BinlogChecksumError> {
        ...
        if !self.crc32_format_description_seen && is_artificial_rotate(event_type, event) {
...
fn verify_crc32(event: &Event) -> Result<(), BinlogChecksumError> {
    let actual = event.checksum().ok_or(BinlogChecksumError::MissingCrc32)?;
    let expected = event
...
pub fn is_artificial_rotate(event_type: EventType, event: &Event) -> bool {
...
impl fmt::Display for BinlogChecksumError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::MissingCrc32 => write!(
                formatter,
                "CRC32 is configured but event checksum is absent"
            ),
            Self::DuplicateBootstrapRotate => {
                write!(
                    formatter,
                    "received more than one pre-FDE artificial rotate event"
                )
            }
            Self::Mismatch { expected, actual } => write!(
                formatter,
                "binlog CRC32 mismatch: expected {expected:02x?}, received {actual:02x?}"
            ),
        }
    }
}
