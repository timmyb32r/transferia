...
use arrow::array::{
    Array, BinaryArray, Decimal128Array, DictionaryArray, Int16Array, Int32Array, Int8Array,
    ListArray, MapArray, StructArray, TimestampMillisecondArray, TimestampSecondArray, UInt8Array,
};
...
fn nested_list_timestamps_preserve_values_offsets_nulls_and_metadata() -> anyhow::Result<()> {
    ...
    let expected =
        DataType::Struct(vec![Arc::new(Field::new("events", expected_list, false))].into());
    let normalized =
        normalize_snapshot_array(&(Arc::new(actual) as ArrayRef), &expected, "payload", false)?;
...
fn dictionary_timestamps_keep_keys_and_nulls() -> anyhow::Result<()> {
    ...
    let normalized = normalize_snapshot_array(
...
fn map_timestamps_keep_entry_order_and_offsets() -> anyhow::Result<()> {
    ...
    let normalized =
        normalize_snapshot_array(&(Arc::new(map) as ArrayRef), &expected, "events", false)?;
...
fn nested_timestamp_rejects_fractional_seconds_with_member_path() -> anyhow::Result<()> {
    ...
    let error = normalize_snapshot_array(&(Arc::new(list) as ArrayRef), &expected, "events", false)
        .unwrap_err()
        .to_string();
    assert!(error.contains("events.item"), "{error}");
    assert!(
        error.contains("cannot be represented losslessly"),
        "{error}"
    );
    Ok(())
}
...
fn native_tuple_names_require_validated_declaration() -> anyhow::Result<()> {
    ...
    let expected =
        DataType::Struct(vec![Arc::new(Field::new("authored", DataType::Int32, false))].into());
...
fn parquet_schema_validation_rejects_plain_integer_timestamp_hint() {
    ...
    let error = validate_parquet_input_schema(&actual, &expected)
        .unwrap_err()
        .to_string();
...
fn parquet_schema_validation_rejects_decimal_scale_drift() {
    let actual = Schema::new(vec![Field::new(
        "amount",
...
fn string_transport_rejects_invalid_utf8_instead_of_replacing_it_with_null() {
    ...
    let error = decode_snapshot_string(&array, "source.payload")
        .unwrap_err()
        .to_string();
...
fn timestamp_normalization_preserves_sliced_list_offsets() -> anyhow::Result<()> {
    ...
    let expected = DataType::List(Arc::new(Field::new("item", second_type(), true)));
    let normalized =
        normalize_snapshot_array(&(Arc::new(list) as ArrayRef), &expected, "events", false)?;
...
#[test]
fn parquet_decode_preserves_nested_metadata_and_normalizes_transport_item_names(
) -> anyhow::Result<()> {
    ...
    let decoded = parquet::arrow::arrow_reader::ParquetRecordBatchReaderBuilder::new_with_metadata(
        bytes, metadata,
    )
    .build()?
    .next()
...
fn enum_labels(array: &ArrayRef) -> Vec<Option<String>> {
    ...
    dictionary
        .keys()
        .iter()
        .map(|key| key.map(|key| values.value(usize::try_from(key).unwrap()).to_owned()))
        .collect()
}
...
fn parquet_nested_enum16_codes_keep_list_order() -> anyhow::Result<()> {
    ...
    let decoded = parquet_enum_round_trip(
        Arc::new(list),
        "Array(Nullable(Enum16('low' = -32768, 'high' = 32767)))",
    )?;
    let list = decoded.as_any().downcast_ref::<ListArray>().unwrap();
    assert_eq!(
        enum_labels(list.values()),
        vec![Some("low".into()), Some("high".into()), None]
    );
    Ok(())
}
...
fn parquet_low_cardinality_enum_preserves_both_dictionary_layers() -> anyhow::Result<()> {
    ...
    let rows = outer
        .keys()
        .iter()
        .map(|key| key.and_then(|key| labels[usize::try_from(key).unwrap()].clone()))
        .collect::<Vec<_>>();
    assert_eq!(
        rows,
        vec![
...
fn parquet_enum_rejects_undeclared_code() {
    let error = parquet_enum_round_trip(
        Arc::new(Int16Array::from(vec![1_i16, -3_i16])),
        "Enum16('one' = 1)",
    )
    .unwrap_err()
    .to_string();
...
fn native_nested_decimal_precision_is_checked_before_reannotation() -> anyhow::Result<()> {
    let values = Arc::new(Decimal128Array::from(vec![1234_i128]).with_precision_and_scale(9, 2)?)
        as ArrayRef;
    let list = ListArray::try_new(
...
fn native_decimal_rejects_value_outside_declared_precision() -> anyhow::Result<()> {
    let array = Arc::new(Decimal128Array::from(vec![10000_i128]).with_precision_and_scale(9, 2)?)
        as ArrayRef;
    let error = normalize_snapshot_array(&array, &DataType::Decimal128(4, 2), "amount", true)
        .unwrap_err()
        .to_string();
...
fn unchanged_nested_decimal_types_still_validate_precision() -> anyhow::Result<()> {
    let values = Arc::new(Decimal128Array::from(vec![10000_i128]).with_precision_and_scale(4, 2)?)
        as ArrayRef;
...
fn native_boolean_requires_verified_type_and_exact_zero_or_one() -> anyhow::Result<()> {
    ...
    let error = normalize_snapshot_array(&invalid, &DataType::Boolean, "enabled", true)
        .unwrap_err()
        .to_string();
