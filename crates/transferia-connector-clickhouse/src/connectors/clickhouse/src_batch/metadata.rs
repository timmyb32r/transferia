...
use crate::connectors::clickhouse::sink::{
    client::ReconnectingClient, table::quote_string_literal,
};
use arrow::{
...
pub(super) fn catalog_query(tables: &[TableIdentity]) -> String {
    let identities = tables
        ...
        .map(|table| {
            format!(
                "({}, {})",
                quote_string_literal(&table.namespace),
...
pub(super) async fn discover_tables(
    ...
    let batches = observe_external_request(
        "clickhouse",
        "load_schema_batch",
        client.query_all(&catalog_query(tables)),
    )
    .await?;
    ...
    for discovered in decoded.values_mut() {
        if let Ok(table) = discovered {
            if let Err(error) = validate_projection(client, table).await {
                *discovered = Err(error);
            }
        }
    }
    Ok(decoded)
...
pub(super) fn decode_batch(
    ...
    let mut groups = tables
        .iter()
        .cloned()
        .map(|table| (table, (Vec::new(), Vec::new())))
        .collect::<BTreeMap<_, _>>();
    for batch in batches {
        anyhow::ensure!(
            batch.num_columns() == 7,
            "ClickHouse metadata batch must contain seven columns"
        );
        ...
        let databases = databases
            ...
            .ok_or_else(|| anyhow::anyhow!("Invalid metadata database names"))?;
        let names = names
            .as_any()
            .downcast_ref::<StringArray>()
            .ok_or_else(|| anyhow::anyhow!("Invalid metadata table names"))?;
        let mut start = 0;
        while start < batch.num_rows() {
            anyhow::ensure!(
                !databases.is_null(start) && !names.is_null(start),
                "ClickHouse metadata table identity is NULL"
            );
            let table = TableIdentity {
                namespace: databases.value(start).into(),
                name: names.value(start).into(),
            };
            let mut end = start + 1;
            while end < batch.num_rows()
                && !databases.is_null(end)
                && !names.is_null(end)
                && databases.value(end) == table.namespace
                && names.value(end) == table.name
            {
                end += 1;
            }
            let (columns, keys) = groups.get_mut(&table).ok_or_else(|| {
                anyhow::anyhow!("ClickHouse returned an unrequested table identity")
            })?;
            columns.push(batch.slice(start, end - start).project(&[2, 3, 4])?);
            if keys.is_empty() {
                keys.push(batch.slice(start, 1).project(&[5, 6])?);
            }
            start = end;
        }
    }
