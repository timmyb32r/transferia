...
`crates/transferia-control-plane/src/server/api_contract.rs` is the only source
of truth for control-plane request
and response shapes. Its Rust DTOs derive `JsonSchema`; the
`generate-server-api` binary materializes that schema as
`crates/transferia-server-contracts/contracts/server-api.schema.json`. The web generator projects the committed
