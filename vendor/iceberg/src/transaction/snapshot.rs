...
pub(crate) trait SnapshotProduceOperation: Send + Sync {
    ...
    /// Whether this operation replaces every row in the table.
...
impl<'a> SnapshotProducer<'a> {
    ...
    pub(crate) fn validate_partition_value(
        ...
        if !self.added_data_files.is_empty() {
            ...
            for file in added_files {
                match file.content_type() {
                    crate::spec::DataContentType::Data => data_files.push(file),
                    crate::spec::DataContentType::PositionDeletes
                    | crate::spec::DataContentType::EqualityDeletes => delete_files.push(file),
                }
            }
            if !data_files.is_empty() {
                manifest_files.push(
                    self.write_added_manifest(ManifestContentType::Data, data_files)
                        .await?,
                );
            }
            if !delete_files.is_empty() {
                manifest_files.push(
                    self.write_added_manifest(ManifestContentType::Deletes, delete_files)
                        .await?,
                );
            }
        }
