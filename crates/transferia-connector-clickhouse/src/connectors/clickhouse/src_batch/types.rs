...
pub(super) fn source_column(
    ...
    let conversion_nullable = parsed.as_ref().map_or(true, |parsed| match parsed {
        Type::LowCardinality(inner) => inner.is_nullable(),
        _ => parsed.is_nullable(),
    });
    let native = parsed
    ...
    Ok(
        SchemaColumn::new(name.to_owned(), data_type, nullable).with_arrow_extension_metadata(
...
pub(super) fn wire_declaration(column: &SchemaColumn) -> Option<String> {
    metadata(column).map(|metadata| {
        if metadata.conversion == SourceConversion::ToString {
            if column.nullable {
                "Nullable(String)"
            } else {
                "String"
...
pub(super) fn validate_wire_declaration(
    actual: &Field,
    expected: Option<&str>,
) -> anyhow::Result<bool> {
    let name = actual.name();
    let Some(actual) = actual.metadata().get("clickhouse.type") else {
        return Ok(false);
    };
    ...
    anyhow::ensure!(
        actual == expected || canonical_declaration(actual) == canonical_declaration(expected),
...
pub(super) fn source_arrow_type(
    ...
    let mapped = match parsed {
        ...
        Type::Tuple(members) => {
            anyhow::ensure!(
                members.len() == args.len(),
                "invalid tuple declaration {declaration}"
            );
            let fields = members
                .iter()
                .zip(&args)
                .enumerate()
                .map(|(index, (member, arg))| {
        ...
        Type::DateTime64(precision, timezone) => {
            ...
            (
                DataType::Timestamp(unit, (args.len() == 2).then(|| Arc::from(timezone.name()))),
                false,
            )
        }
        ...
        Type::Point | Type::Ring | Type::Polygon | Type::MultiPolygon => {
            let (data_type, nullable) =
                clickhouse_arrow::arrow::ch_to_arrow_type(parsed, Some(ArrowOptions::strict()))?;
        ...
        | Type::Decimal256(scale)
            if declaration.trim_start().starts_with("Decimal(") =>
        {
...
fn argument<'a>(arguments: &[&'a str], index: usize) -> anyhow::Result<&'a str> {
    arguments
        .get(index)
        .copied()
        .ok_or_else(|| anyhow::anyhow!("missing type argument {index}"))
}
...
fn arguments(declaration: &str) -> anyhow::Result<Vec<&str>> {
    ...
    let body = declaration[open + 1..]
        .strip_suffix(')')
        .ok_or_else(|| anyhow::anyhow!("unclosed type declaration"))?;
    ...
    while let Some((offset, ch)) = chars.next() {
        ...
        match ch {
            ...
            ')' => {
                depth = depth
                    .checked_sub(1)
                    .ok_or_else(|| anyhow::anyhow!("unbalanced type declaration"))?;
            }
            ',' if depth == 0 => {
                result.push(body[start..offset].trim());
                start = offset + 1;
            }
            _ => {}
        }
...
fn tuple_member(argument: &str, index: usize) -> anyhow::Result<(String, &str)> {
    ...
    if Type::from_str(argument).is_ok() {
        return Ok(((index + 1).to_string(), argument));
    }
    let (name, rest) = if argument.starts_with(['`', '"']) {
        ...
        let end = argument
            ...
            .ok_or_else(|| anyhow::anyhow!("missing tuple member type"))?;
        (argument[..end].to_owned(), &argument[end..])
    };
    anyhow::ensure!(
        rest.starts_with(char::is_whitespace),
        "missing space after tuple member name"
    );
