...
fn source_quoted_keys_preserve_names_and_do_not_split_quoted_commas() {
    ...
    let mut schema = DatasetSchema::new(
        names
            .iter()
            .map(|name| SchemaColumn::new((*name).into(), DataType::Int64, false))
            .collect(),
    );
    ...
    assert!(
        crate::connectors::clickhouse::sink::identifier::validate_identifier("entries.query_id")
            .is_err()
    );
}
...
fn source_quoted_keys_still_reject_expressions_and_malformed_lists() {
    ...
    for key in [
        ...
        assert!(
            apply_discovered_primary_key(&mut schema, &table, key, key).is_err(),
            "{key}"
        );
    }
}
...
fn column_kind_readable_defaults_do_not_block_source_discovery() {
    ...
    for kind in ["", "DEFAULT", "MATERIALIZED", "ALIAS"] {
        super::super::connector::validate_source_column_kind(&table, "databases", kind).unwrap();
    }
}
...
fn column_kind_ephemeral_fails_with_specific_table_and_column_context() {
    ...
    let error =
        super::super::connector::validate_source_column_kind(&table, "input_only", "EPHEMERAL")
            .unwrap_err()
            .to_string();
    for expected in [
        "analytics",
...
fn column_kind_unknown_fails_closed() {
    ...
    assert!(
        super::super::connector::validate_source_column_kind(&table, "value", "FUTURE_KIND",)
            .is_err()
    );
}
...
fn reported_information_schema_enum_is_supported_without_conversion() {
    ...
    let column =
        types::source_column("is_updatable", declaration, UnsupportedTypePolicy::Fail).unwrap();
    assert_eq!(
        column.data_type,
...
fn unsupported_types_follow_the_selected_conversion_policy() {
    for declaration in [
        ...
        assert!(
            types::source_column("value", declaration, UnsupportedTypePolicy::Fail).is_err(),
            "{declaration}"
        );
        let column =
            types::source_column("value", declaration, UnsupportedTypePolicy::ToString).unwrap();
...
fn recursive_types_preserve_named_tuple_fields_and_timezone() {
    ...
    let DataType::List(item) = column.data_type else {
        panic!("expected list")
    };
...
fn wire_metadata_must_match_before_restoring_tuple_names() {
    let column =
        types::source_column("value", "Tuple(a Int64)", UnsupportedTypePolicy::Fail).unwrap();
    let field = Field::new("value", column.data_type.clone(), false).with_metadata(
        std::collections::HashMap::from([("clickhouse.type".into(), "Tuple(b Int64)".into())]),
    );
...
fn source_policy_defaults_to_string_and_preserves_explicit_fail() {
    ...
    let config: super::super::config::ClickHouseSourceConfig =
        serde_json::from_value(value.clone()).unwrap();
    assert_eq!(config.unsupported_types, UnsupportedTypePolicy::ToString);
    assert_eq!(
        types::source_column("payload", "Dynamic", config.unsupported_types)
            .unwrap()
            .data_type,
        DataType::Utf8
    );
    value["unsupported_types"] = serde_json::json!("fail");
    let strict: super::super::config::ClickHouseSourceConfig =
    ...
    let schema = serde_json::to_value(schemars::schema_for!(
        super::super::config::ClickHouseSourceConfig
    ))
    .unwrap();
    assert_eq!(
        schema["properties"]["unsupported_types"]["x-ui"]["section"],
        "advanced"
    );
    assert_eq!(
        schema["properties"]["unsupported_types"]["default"],
        "to_string"
    );
    assert_eq!(
...
fn decimal_precision_and_scale_are_not_widened_to_storage_capacity() {
    for (declaration, expected) in [
        ("Decimal(4, 0)", DataType::Decimal128(4, 0)),
        ("Decimal(40, 12)", DataType::Decimal256(40, 12)),
    ] {
        let column =
            types::source_column("amount", declaration, UnsupportedTypePolicy::Fail).unwrap();
...
fn anonymous_tuple_and_geo_names_follow_clickhouse_numbering() {
    for declaration in ["Tuple(Float64, Float64)", "Point"] {
        let column =
            types::source_column("point", declaration, UnsupportedTypePolicy::Fail).unwrap();
    ...
    let column = types::source_column(
        "point",
        "Tuple(field_0 Float64, field_1 Float64)",
...
fn snapshot_query_converts_only_opted_in_columns_and_guards_original_types() {
    use super::super::{
        config::TableConfig,
        connector::{snapshot_query, DiscoveredTable},
    };
    use transferia_core::data::schema::DatasetSchema;
    use transferia_core::SystemColumns;
    let table = DiscoveredTable {
        ...
        physical_system_columns: SystemColumns::default(),
    };
...
fn malformed_tuple_identifiers_and_remaining_types_fail_closed() {
    for declaration in [
        ...
        r"Tuple(`\xD0` String)",
        r"Tuple(`\x0` String)",
        r"Tuple(`unclosed String)",
        "Tuple(`name`Int64)",
        "Tuple(`name` Int64 garbage)",
        "Tuple(\"name\"\" String)",
    ] {
        assert!(
            types::source_column("value", declaration, UnsupportedTypePolicy::Fail).is_err(),
...
fn native_scalar_cases() -> Vec<(String, DataType)> {
    ...
    let mut cases = vec![
        ...
        ("FixedString(17)", DataType::FixedSizeBinary(17)),
        ...
        (
            ...
            DataType::Timestamp(TimeUnit::Second, Some("Europe/Moscow".into())),
        ),
        (
            "Enum8('negative' = -128, 'zero' = 0, 'positive' = 127)",
            DataType::Dictionary(Box::new(DataType::Int32), Box::new(DataType::Utf8)),
        ),
        (
            "Enum16('negative' = -32768, 'zero' = 0, 'positive' = 32767)",
            DataType::Dictionary(Box::new(DataType::Int32), Box::new(DataType::Utf8)),
        ),
    ]
    .into_iter()
    .map(|(declaration, data_type)| (declaration.to_owned(), data_type))
    .collect::<Vec<_>>();
    for precision in 0..=9 {
        ...
        cases.push((
            format!("DateTime64({precision}, 'Asia/Kolkata')"),
            DataType::Timestamp(unit, Some("Asia/Kolkata".into())),
        ));
    }
    cases
}
...
fn assert_native_contract(declaration: &str, expected: &DataType, nullable: bool) {
    for policy in [UnsupportedTypePolicy::Fail, UnsupportedTypePolicy::ToString] {
        let column = types::source_column("source_value", declaration, policy)
        ...
        assert!(
            !types::is_string_conversion(&column),
            "supported {declaration} must remain native even when conversion is enabled"
        );
        assert_eq!(
            types::source_declaration(&column).as_deref(),
            Some(declaration)
        );
        assert_eq!(
            types::wire_declaration(&column).as_deref(),
            Some(declaration)
        );
...
fn native_scalar_family_and_recursive_modifier_matrix_preserves_declared_contracts() {
    ...
    for (declaration, data_type) in native_scalar_cases() {
        ...
        assert_native_contract(
            &format!("Array(Nullable({declaration}))"),
            &DataType::List(Arc::new(Field::new("item", data_type.clone(), true))),
            false,
        );
        assert_native_contract(
            &format!("Tuple(value Nullable({declaration}), marker UInt8)"),
            &DataType::Struct(
                vec![
                    Field::new("value", data_type.clone(), true),
                    Field::new("marker", DataType::UInt8, false),
                ]
                .into(),
            ),
            false,
        ...
        assert_native_contract(
            &format!("Map(String, Nullable({declaration}))"),
            &DataType::Map(
                Arc::new(Field::new(
                    "entries",
                    DataType::Struct(
...
fn low_cardinality_scalar_matrix_preserves_dictionary_and_nullability() {
    for (declaration, data_type) in native_scalar_cases()
        ...
        .filter(|(declaration, _)| {
            matches!(
                declaration.as_str(),
                "String" | "FixedString(17)" | "Date" | "DateTime"
            )
        })
    {
        let dictionary = DataType::Dictionary(Box::new(DataType::Int32), Box::new(data_type));
        assert_native_contract(
            &format!("LowCardinality({declaration})"),
            &dictionary,
            false,
        );
        assert_native_contract(
            &format!("LowCardinality(Nullable({declaration}))"),
            &dictionary,
            true,
        );
        assert_native_contract(
            &format!("Array(LowCardinality(Nullable({declaration})))"),
            &DataType::List(std::sync::Arc::new(Field::new("item", dictionary, true))),
            false,
        );
    }
}
...
fn composite_native_families_remain_native_through_nested_shapes() {
    for declaration in [
        ...
        let original =
            types::source_column("source_value", declaration, UnsupportedTypePolicy::Fail).unwrap();
...
fn unsupported_family_and_recursive_modifier_matrix_requires_whole_column_opt_in() {
    for declaration in [
        ...
        "Nested(name String, amount Decimal(10,2))",
        "Time",
        "Time64(6)",
        "QBit(Float32, 16)",
        "Variant(String, UInt64)",
        ...
        for shape in [
            ...
            assert!(
                types::source_column("source_value", &shape, UnsupportedTypePolicy::Fail).is_err(),
                "{shape}"
            );
            let converted =
                types::source_column("source_value", &shape, UnsupportedTypePolicy::ToString)
                    .unwrap();
            assert_eq!(
                converted.data_type,
                DataType::Utf8,
                "the entire {shape} column is converted"
            );
            assert!(types::is_string_conversion(&converted), "{shape}");
            assert_eq!(
                types::source_declaration(&converted).as_deref(),
                Some(shape.as_str())
            );
            assert_eq!(
                types::wire_declaration(&converted).as_deref(),
                Some(if converted.nullable {
                    "Nullable(String)"
                } else {
                    "String"
                })
            ...
            let metadata: serde_json::Value =
                serde_json::from_str(converted.arrow_extension_metadata.as_deref().unwrap())
                    .unwrap();
