...
fn topic_settings(partitions_count: i32, consumers: &[&str]) -> TopicSettings {
    TopicSettings {
        ...
        read_rules: consumers
            .iter()
            .map(|consumer_name| {
                crate::connectors::logbroker::proto::pers_queue::v1::topic_settings::ReadRule {
...
fn connector(config: &str) -> anyhow::Result<PqV1SourceConnector> {
    let value = serde_yaml::from_str(config)?;
    PqV1SourceConnector::from_config(value, Arc::new(MetricsRegistry::new()))
}
...
fn validates_static_partition_group_ids() {
    for (ids, expected) in [
        ...
        let error = connector(&config(ids, "")).err().expect("config must fail");
...
fn discovered_topic_requires_a_stable_partition_topology() {
    for strategy in [
        ...
        settings.auto_partitioning_settings = Some(
            crate::connectors::logbroker::proto::pers_queue::v1::AutoPartitioningSettings {
...
fn rejects_unknown_host_field() {
    ...
    assert!(connector(&invalid).is_err());
}

#[tokio::test]
async fn rejects_builds_for_undeclared_partitions_before_network_io() {
    let source = connector(&config("partition_group_ids: [0]\n", "")).unwrap();
    let error = source
...
fn reports_benchmark_discard_behavior() {
    for cfg in [
        ...
        let source = connector(&cfg).unwrap();
        let endpoint = source.compatibility(transferia_delivery_contracts::DeliveryType::Stream);
        ...
        let discovery = source
            ...
            .unwrap();
        assert!(transferia_delivery_contracts::semantics::validate_pipeline(
            &endpoint,
...
fn configured_discovery_uses_the_parser_projection() -> anyhow::Result<()> {
    let source = connector(&json_config(""))?;
    ...
    assert_eq!(
        ...
        transferia_core::delivery::SchemaOrigin::ParserProjection
    );
    ...
    assert_eq!(
        discovery
            .dataset(transferia_core::delivery::DatasetRole::DeadLetterQueue)?
            .name
            .as_ref(),
        "events_dlq"
    );
...
fn payload_discard_requires_the_discard_parser() {
    let error = connector(&json_config(
        "benchmark_discard_before_decompression: true\n",
    ))
    .err()
...
fn missing_partition_group_ids_are_discovered_later() {
    let source = connector(&config("", "")).expect("dynamic topology must be accepted");
    assert!(source.cfg.partition_group_ids.is_empty());
}

#[test]
fn static_partitions_are_split_without_truncating_ids() {
    let source = connector(&config("partition_group_ids: [0, 1, 4294967297]\n", "")).unwrap();
    let discovery = source
        .configured_delivery_discovery(DeliveryDiscoveryRequest {
            keep_system_columns: false,
        })
...
fn retries_reuse_partition_source_counters() {
    let source = connector(&config("partition_group_ids: [0, 1]\n", "")).unwrap();
...
fn cached_endpoint_order_remains_partition_specific() {
    ...
    let endpoints = vec![
        crate::connectors::logbroker::proto::discovery::EndpointInfo {
            address: "a.test".into(),
            port: 2135,
            load_factor: 0.0,
            ..Default::default()
