...
fn all_tables_and_independent_pattern_modes_share_startup_and_admission_rules() {
    ...
    let selected = TableSelection::Selected {
        rules: vec![TableRule {
            include: "db.reports_*".into(),
            include_mode: PatternMode::Glob,
            exclude: Some(r"db\.reports_(test|temp)".into()),
            exclude_mode: PatternMode::Regex,
        }],
    };
...
fn all_tables_is_a_fieldless_strict_variant_in_config_and_schema() {
    ...
    let all = branches
        .iter()
        .find(|branch| branch["properties"]["type"]["const"] == "all")
        .unwrap();
    assert_eq!(
        all["properties"]
            .as_object()
...
fn completion_globs_match_from_the_beginning_not_a_substring() {
    let catalog = [
        ...
        table("information_schema", "reports"),
        table("Schema", "reports"),
    ];
...
fn exclusions_are_limited_to_their_own_include() {
    ...
    let selection = TableSelection::Selected {
        rules: vec![
            rule("db.reports_*", Some("db.*_test")),
            rule("db.users_test", None),
        ],
    };
...
fn two_cards_selecting_the_same_table_are_a_conflict() {
    ...
    let result = resolve(
        vec![rule("db.*", None), rule("db.reports_*", None)],
        std::slice::from_ref(&target),
    );
    assert_eq!(
        result.issues,
        vec![SelectionIssue::Conflict {
            table: target,
            first_card: 0,
            second_card: 1,
            kind: ConflictKind::MultipleIncludes,
        }]
    );
}
...
fn another_cards_exclusion_conflicts_in_either_order() {
    ...
    .enumerate()
    {
        let result = TableSelection::Selected { rules }
            .compile()
            .unwrap()
            .resolve(std::slice::from_ref(&target))
            .unwrap();
        assert_eq!(
            result.issues,
            vec![
                SelectionIssue::Conflict {
                    ...
                    first_card: 0,
                    second_card: 1,
                    kind: ConflictKind::IncludeExclude,
                },
...
fn two_exclusions_do_not_conflict() {
    ...
    assert_eq!(
        result.issues,
        vec![
            SelectionIssue::EmptyMatch { card: 0 },
            SelectionIssue::EmptyMatch { card: 1 }
        ]
    );
}
...
fn empty_policy_applies_after_exclusion_to_every_card() {
    let result = resolve(
        vec![rule("db.*", Some("db.*")), rule("other.*", None)],
        &[table("db", "t")],
    );
    assert_eq!(
        result.issues,
...
fn regex_matches_entire_name_including_newlines() {
    let selection = TableSelection::Selected {
        rules: vec![TableRule {
            include: r"db\.reports_[0-9]+".into(),
            exclude: None,
            include_mode: PatternMode::Regex,
            exclude_mode: PatternMode::Glob,
        }],
    };
    let catalog = [
...
fn escaped_glob_names_preserve_literal_special_characters() {
    ...
    let result = resolve(
        vec![rule(&include, None)],
        &[target.clone(), table("db", "aZZbXc\\d")],
    );
...
fn exact_globs_leave_dots_literal_but_regex_escapes_them() {
    ...
    let result = resolve(
        vec![rule(&PatternMode::Glob.exact_pattern(&target), None)],
        &catalog,
    );
    assert_eq!(result.cards[0].selected, vec![target]);
    assert_eq!(
        PatternMode::Glob.exact_pattern(&table("a.b", "c")),
        r"a\\.b.c"
    );
    assert_eq!(
        PatternMode::Glob.exact_pattern(&table("a", "b.c")),
...
fn invalid_patterns_fail() {
    for bad in [
        ...
        assert!(TableSelection::Selected { rules: vec![bad] }
            .compile()
            .is_err());
    }
}
...
fn exact_patterns_roundtrip_in_both_modes() {
    ...
    for mode in [PatternMode::Glob, PatternMode::Regex] {
        let result = TableSelection::Selected {
            rules: vec![TableRule {
                include: mode.exact_pattern(&target),
                exclude: None,
                include_mode: mode,
                exclude_mode: PatternMode::Glob,
            }],
...
fn qualified_names_do_not_merge_dots_in_different_identifier_parts() {
    ...
    for mode in [PatternMode::Glob, PatternMode::Regex] {
        ...
        .compile()
        .unwrap()
        .resolve(&[target.clone(), other.clone()])
        .unwrap();
        assert_eq!(result.cards[0].selected, vec![target.clone()]);
    }
}
...
fn serialized_defaults_fail_on_empty_matches_and_use_glob() {
    ...
    .unwrap();
    assert_eq!(
        selection.compile().unwrap().resolve(&[]).unwrap().issues,
        vec![SelectionIssue::EmptyMatch { card: 0 }]
    );
    assert!(serde_json::from_value::<TableSelection>(serde_json::json!({
        "type": "selected", "rules": [], "empty_matches": "allow_empty_matches"
    }))
    .is_err());
    assert!(serde_json::to_value(schemars::schema_for!(TableSelection))
        .unwrap()
        .pointer("/properties/empty_matches")
        .is_none());
    assert_eq!(
        match selection {
            TableSelection::Selected { rules } => rules[0].include_mode,
            TableSelection::All { .. } => panic!("selected"),
        },
        PatternMode::Glob
    );
}
...
fn classifying_a_future_table_uses_the_same_conflicts_as_preview() {
    ...
    assert_eq!(
        compiled
            ...
            .unwrap()
            .issues,
        compiled.classify(&target).issues
    );
}
...
fn startup_cannot_consume_a_conflicting_preview() {
    let preview = resolve(
        vec![rule("db.*", None), rule("db.t", None)],
        &[table("db", "t")],
    );
    assert!(preview.selected_tables().is_err());
}
...
fn no_rules_rejects_startup() {
    let preview = TableSelection::Selected { rules: vec![] }
        .compile()
        .unwrap()
        .resolve(&[])
        .unwrap();
...
fn qualified_identity_parser_roundtrips_literal_dots_backslashes_and_unicode() -> anyhow::Result<()>
{
    for namespace in ["", "public", r"name.space\🦀"] {
        for name in ["events", r"event.part\🦀", "  untouched  "] {
            ...
            assert_eq!(
                TableIdentity::from_qualified_name(&table.qualified_name())?,
                table
            );
        }
    }
    for invalid in [
        ...
        assert!(
            TableIdentity::from_qualified_name(invalid).is_err(),
            "{invalid:?}"
        );
    }
    Ok(())
