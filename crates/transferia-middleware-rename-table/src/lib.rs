...
use transferia_registry::{
    qualified_table_name, MiddlewareRegistration, RegistryBuilder, TableIdentity,
};
...
fn validate_name(name: &str) -> anyhow::Result<()> {
    anyhow::ensure!(
        !name.trim().is_empty(),
        "Rename table: resulting name must not be empty"
    );
    anyhow::ensure!(
        !name.contains('\0'),
        "Rename table: resulting name must not contain NUL"
    );
    Ok(())
}
...
impl RenameTableMiddleware {
        ...
        let (rename, last_part_only) = match config {
            ...
            RenameTableConfig::Regex {
                ...
                anyhow::ensure!(
                    ...
                    "Rename table: replacement must not contain NUL"
                );
    ...
    fn renamed(
        ...
        namespace: Option<&Arc<str>>,
        name: &Arc<str>,
    ) -> anyhow::Result<(Option<Arc<str>>, Arc<str>)> {
        match &self.rename {
            Rename::Exact {
            ...
            Rename::Regex {
                ...
                let input = if self.last_part_only {
                    ...
                    std::borrow::Cow::Owned(qualified_table_name(
                        namespace.map(AsRef::as_ref),
                        name,
                    ))
                };
                ...
                for captures in pattern.captures_iter(&input) {
                    let entire = captures.get(0).ok_or_else(|| {
                        anyhow::anyhow!("Rename table: regex returned no whole match")
                    })?;
                    matched = true;
                ...
                if !matched {
                    return Ok((namespace.cloned(), Arc::clone(name)));
                }
                output.push_str(&input[offset..]);
                validate_name(&output)?;
                if self.last_part_only {
                    Ok((namespace.cloned(), output.into()))
                } else {
...
fn parse_identity(value: &str) -> anyhow::Result<(Option<Arc<str>>, Arc<str>)> {
    ...
    Ok((
        (!identity.namespace.is_empty()).then(|| Arc::from(identity.namespace)),
...
fn parse_replacement(pattern: &Regex, text: &str) -> anyhow::Result<Vec<ReplacementPart>> {
    ...
    while let Some(dollar) = rest.find('$') {
        ...
        if let Some(braced) = rest.strip_prefix('{') {
            let end = braced
                .find('}')
                .ok_or_else(|| anyhow::anyhow!("Rename table: unclosed capture reference"))?;
        ...
        anyhow::ensure!(
            !reference.is_empty(),
            "Rename table: use $$ for a literal dollar"
        );
        let index = reference
            .parse::<usize>()
            .ok()
            .filter(|index| *index < pattern.captures_len())
            .or_else(|| {
                pattern
                    .capture_names()
                    .position(|name| name == Some(reference))
            })
...
impl Middleware for RenameTableMiddleware {
    ...
    fn output_table_identity(
        ...
        self.renamed(namespace.map(Arc::from).as_ref(), &Arc::from(name))
    }
    ...
    async fn process(&self, mut data: TableData) -> anyhow::Result<TableData> {
        (data.namespace, data.table) = self.renamed(data.namespace.as_ref(), &data.table)?;
        Ok(data)
    }
}
