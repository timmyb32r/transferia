...
#[test]
fn source_sample_decode_obeys_its_budget_before_materializing_the_full_payload(
) -> anyhow::Result<()> {
    ...
    for (codec, payload) in [
        ...
        let error =
            super::source::decode_message_bounded(codec, payload.clone(), 1024).unwrap_err();
        assert!(error.to_string().contains("exceeds 1024 bytes"));
        assert_eq!(
            super::source::decode_message_bounded(codec, payload, data.len())?.as_ref(),
            data
        );
    }
    Ok(())
