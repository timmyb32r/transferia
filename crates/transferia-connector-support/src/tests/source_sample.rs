...
fn message() -> Message {
    Message {
        ...
        headers: Arc::from([MessageHeader {
            key: Arc::from("header"),
            value: None,
        }]),
        meta: MessageMeta {
...
async fn sample_matches_the_configured_production_parser_including_envelope() -> anyhow::Result<()>
{
    ...
    input.value = bytes::Bytes::new();
    let expected = plan
        .parser()
        .create_session(limits().max_bytes)
        .parse_into(vec![input.clone()])?
        .0;
    let actual = parse_sample(
        plan.parser(),
        vec![input],
        limits(),
...
async fn sample_rejects_insufficient_budget_and_cancellation() -> anyhow::Result<()> {
    ...
    cancellation.cancel();
    assert!(
        parse_sample(plan.parser(), vec![message()], limits(), cancellation)
            .await
            .is_err()
    );
    Ok(())
