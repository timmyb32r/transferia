...
use transferia_core::data::{
    message::Message,
    schema::{DatasetSchema, SchemaColumn},
    table_data::{dlq_name, TableData},
};
...
pub(crate) fn message_dlq_schema() -> DatasetSchema {
    DatasetSchema::new(vec![
        ...
        SchemaColumn::new(
            "source_write_timestamp_ms".to_owned(),
            DataType::Int64,
            true,
        ),
            ...
            sum.saturating_add(header.key.len().saturating_mul(6))
                .saturating_add(
                    header
                        .value
                        .as_ref()
                        .map_or(0, |value| value.len().saturating_mul(2)),
                )
                .saturating_add(16)
        });
        sum.saturating_add(payload.saturating_mul(4))
            .saturating_add(headers.saturating_mul(3))
...
pub(crate) fn rejected_messages(
    table: &str,
    messages: &[Message],
    memory_limit: usize,
) -> anyhow::Result<Option<TableData>> {
    if messages.is_empty() {
        return Ok(None);
    }
    anyhow::ensure!(
        message_dlq_bound(messages) <= memory_limit,
        "parser DLQ exceeds pipeline memory budget"
    );
    let base64 = base64::engine::general_purpose::STANDARD;
    let raw = messages
        .iter()
        .map(|m| base64.encode(&m.value))
        .collect::<Vec<_>>();
    let keys = messages
        .iter()
        .map(|m| m.key.as_ref().map(|key| base64.encode(key)))
        .collect::<Vec<_>>();
            ...
            serde_json::to_string(
                &m.headers
                    ...
                    .map(|h| {
                        (
                            h.key.as_ref(),
                            h.value.as_ref().map(|value| base64.encode(value)),
                        )
                    })
                    .collect::<Vec<_>>(),
            )
