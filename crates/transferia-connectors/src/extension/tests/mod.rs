...
#[async_trait]
impl DynamicOptionsConnector for EmptyOptions {
    async fn list(
        &self,
...
struct UnknownConnectorExtension;

impl TransferiaExtension for UnknownConnectorExtension {
    fn identity(&self) -> ExtensionIdentity {
        ExtensionIdentity {
            package: "test-unknown-connector",
            abi_version: 1,
        }
    }
...
#[test]
fn composition_rejects_unknown_connector_roles() {
    let Err(error) = TransferiaBuilder::new()
        .with_extension(Arc::new(UnknownConnectorExtension))
        .build()
    else {
        panic!("unknown connector role unexpectedly compiled");
    };
    assert!(error.to_string().contains("unknown connector role"));
}
...
#[async_trait]
impl DynamicOptionsConnector for BlockingOptions {
    async fn list(
        &self,
        _request: OptionsRequest,
        ...
        context.cancellation.cancelled().await;
        anyhow::bail!("options connector observed cancellation")
    }
}
