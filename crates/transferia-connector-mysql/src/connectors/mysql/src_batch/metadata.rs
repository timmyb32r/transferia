...
pub(super) fn catalog_query(count: usize, mysql8: bool) -> String {
    ...
    let requested = (0..count)
        .map(|index| {
            format!("SELECT {index} AS request_index, ? AS requested_schema, ? AS requested_table")
        })
        .collect::<Vec<_>>()
        .join(" UNION ALL ");
    let (srs, collation, padding) = if mysql8 {
        ("c.SRS_ID", "col.ID", "col.PAD_ATTRIBUTE")
    } else {
        ("NULL", "NULL", "NULL")
    };
...
pub(super) async fn discover_tables(
    ...
    let parameters = tables
        ...
        .flat_map(|table| {
            [
                Value::from(table.namespace.clone()),
                Value::from(table.name.clone()),
            ]
        })
        .collect();
    let rows: Vec<Row> = observe_external_request(
        "mysql",
        "load_schema_batch",
        connection.exec(
            catalog_query(tables.len(), mysql8),
    ...
    for row in rows {
        let index = row
            .get_opt::<u64, _>("request_index")
            .ok_or_else(|| anyhow::anyhow!("MySQL metadata has no request ordinal"))??;
        let index = usize::try_from(index)?;
        let group = groups
            .get_mut(index)
            .ok_or_else(|| anyhow::anyhow!("MySQL metadata returned an unknown request ordinal"))?;
        group.push(row);
    }
    Ok(tables
        .iter()
...
fn field<T: mysql_async::prelude::FromValue>(row: &Row, name: &str) -> anyhow::Result<T> {
    row.get_opt(name)
        .ok_or_else(|| anyhow::anyhow!("MySQL metadata has no {name}"))?
        .map_err(Into::into)
}
...
fn decode_table(
    ...
    let first = rows.first().ok_or_else(|| {
        anyhow::anyhow!(
            "MySQL table '{}' returned no metadata",
            table.qualified_name()
        )
    })?;
    let engine = field::<Option<String>>(first, "ENGINE")?.ok_or_else(|| {
        anyhow::anyhow!(
    ...
    for row in &rows {
        ...
        let name = field::<Option<String>>(row, "COLUMN_NAME")?.ok_or_else(|| {
            anyhow::anyhow!(
        ...
        anyhow::ensure!(
            names.insert(name),
            "MySQL table '{}' returned duplicate column metadata",
            table.qualified_name()
        );
    }
    assemble_table(
