...
fn snapshot_uses_cdc_schema_zero_version_and_no_fabricated_timestamp() {
    ...
    fields
        .extend(kinds.map(|kind| {
            arrow::datatypes::Field::new(kind.default_name(), kind.data_type(), false)
        }));
    let input = TableData::new(
        Arc::from("t"),
        false,
        RecordBatch::try_new(
            ...
            vec![
                ...
                Arc::new(StringArray::from(vec!["/db/t"])),
    ...
    let version = result
        .batch
        .column_by_name("_system_source_version")
        .unwrap()
        .as_any()
    ...
    assert_eq!(
        result
            .batch
            .column_by_name("_system_source_timestamp_ms")
            .unwrap()
            .null_count(),
        1
    );
    assert_eq!(
        result
            .batch
            .column_by_name("_system_source_transaction_id")
            .unwrap()
            .null_count(),
        1
    );
    assert_eq!(
        result
            .batch
            .column_by_name("_system_write_timestamp_ms")
            .unwrap()
            .null_count(),
        1
    );
    ...
    assert_eq!(
        result
            .batch
            .column_by_name("_system_change_operation")
            .unwrap()
            .as_any()
