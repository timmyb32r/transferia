...
it("opens over the editor, focuses Close and restores the launcher and scrolling on close", () => {
  function Fixture() {
    ...
    return <><button onClick={() => setOpen(true)}>Data schema</button><p>Editor remains mounted</p>
      {open && <DataSchemaDialog result={result} onClose={() => setOpen(false)} />}</>;
  }
  const view = render(<Fixture />);
  const launcher = view.getByRole("button", { name: "Data schema" });
  const editor = view.getByText("Editor remains mounted");
  const overflow = document.documentElement.style.overflow;
  launcher.focus(); fireEvent.click(launcher);
  const dialog = view.getByRole("dialog", { name: "Data schema" });
  expect(dialog.getAttribute("aria-modal")).toBe("true");
  expect(document.activeElement).toBe(view.getByRole("button", { name: "Close Data schema" }));
  expect(document.documentElement.style.overflow).toBe("hidden");
  expect(view.getByText("Editor remains mounted")).toBe(editor);
  fireEvent.click(view.getByRole("button", { name: "Close Data schema" }));
...
it("keeps the popup and close control mounted while loading, showing results or errors", () => {
  ...
  const button = view.getByRole("button", { name: "Close Data schema" });
  ...
  expect(view.getByRole("button", { name: "Close Data schema" })).toBe(button);
