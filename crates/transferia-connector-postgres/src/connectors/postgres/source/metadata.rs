...
pub(super) async fn discover_tables(
    ...
    observe_external_request(
        "postgres",
        "begin_metadata_batch",
        client.batch_execute("BEGIN TRANSACTION ISOLATION LEVEL REPEATABLE READ READ ONLY"),
    )
    .await?;
    let result = discover_in_transaction(client, tables, policy).await;
    let rollback = observe_external_request(
        "postgres",
        "finish_metadata_batch",
...
async fn discover_in_transaction(
    ...
    let rows = observe_external_request(
        "postgres",
        "load_schema_batch",
        client.query(CATALOG_QUERY, &[&namespaces, &names]),
    )
    .await?;
    ...
    for row in rows {
        ...
        let index = usize::try_from(ordinal)?
            .checked_sub(1)
            .ok_or_else(|| anyhow::anyhow!("Invalid PostgreSQL metadata ordinal"))?;
        groups
            .get_mut(index)
            .ok_or_else(|| {
                anyhow::anyhow!("PostgreSQL metadata returned an unknown request ordinal")
            })?
            .push(row);
    }
    ...
    if !projections.is_empty() {
        ...
        drop(
            observe_external_request(
                "postgres",
                "validate_schema_batch_projection",
...
pub(super) fn catalog_type(
    ...
    anyhow::ensure!(
        matches!(kind, "b" | "c" | "e" | "r" | "m" | "p"),
        "Unsupported PostgreSQL catalog type kind {kind:?} for '{name}'"
    );
    if let Some(native) = Type::from_oid(oid) {
        return Ok(native);
    }
...
fn decode_table(
    ...
    let first = rows.first().ok_or_else(|| {
        anyhow::anyhow!(
            "PostgreSQL table '{}' returned no metadata",
            table.qualified_name()
        )
    })?;
    let relation_oid: u32 = first
        .try_get::<_, Option<u32>>("relation_oid")?
        .ok_or_else(|| {
            anyhow::anyhow!(
    ...
    for row in rows {
        anyhow::ensure!(
            row.try_get::<_, u32>("relation_oid")? == relation_oid
                && row.try_get::<_, String>("replica_identity")? == replica_identity,
            "PostgreSQL table identity changed within metadata batch"
        );
        let name = row
        ...
        anyhow::ensure!(
            position > previous_position && names.insert(name.clone()),
            "PostgreSQL table '{}' returned unordered or duplicate columns",
            table.qualified_name()
        );
        previous_position = position;
        let nullable = row
            .try_get::<_, Option<bool>>("nullable")?
            .ok_or_else(|| anyhow::anyhow!("missing nullability metadata for column '{name}'"))?;
        // RowDescription unwraps domains, but CDC identity uses the physical OID.
        let data_type = catalog_type(
            row.try_get("effective_oid")?,
            row.try_get("type_name")?,
            &row.try_get::<_, String>("type_kind")?,
